function __bobthefish_colors -S -a color_scheme -d 'Define colors used by bobthefish'
  switch "$color_scheme"
    ...
    case '*' # default dark theme
      ...
      set -l orange   f6b117 unused 3a2a03
      set -l brown    bf5e00 803f00 4d2600
      set -l grey     cccccc C0C0C0 333333
      set -l white    ffffff
      set -l black    000000
      set -l ruby_red af0000
      ...
      set -x color_path                     $purple[1] $black[1]
      set -x color_path_basename            $purple[1] $black --bold
      set -x color_path_nowrite             $red[3] $white
      set -x color_path_nowrite_basename    $red[3] $white --bold

      set -x color_repo                     $green[2] $white
      set -x color_repo_work_tree           $grey[3] $white --bold
      set -x color_repo_dirty               $red[3] $white
      set -x color_repo_staged              $orange[1] white --bold

      set -x color_vi_mode_default          $grey[2] $white --bold
      set -x color_vi_mode_insert           $green[2] $white --bold
      set -x color_vi_mode_visual           $orange[1] $orange[3] --bold
